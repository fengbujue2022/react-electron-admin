...
export const createHashRehistory = <T = LocationState>(
...
export type RelocationListener<S = LocationState> = (p: {
  location: Location<S>;
  action: Reaction;
}) => void;
...
const createRehistory = <HistoryLocationState = LocationState>(
    ...
    if (isAllowTx) {
      if (action === 'push') {
        locationChain = locationChain
          .splice(0, activeChainIndex + 1)
          .concat([history.location]);
        activeChainIndex += 1;
      } else if (action === 'replace') {
        locationChain = locationChain
          .splice(0, activeChainIndex)
          .concat([history.location]);
  ...
  return {
    ...
    go,
    block(
      blocker?: boolean | string | TransitionPromptHook<HistoryLocationState>
    ): UnregisterCallback {
