...
export class BaseLeanCloudRepository<TEntity extends BaseEntity> {
  ...
  async getPagingList(
    ...
    castRelatedDataMap?: (pagingResult: AV.Queriable[]) => {
      [key in string]: any[];
    }
  ...
  async getOneById(objectId: string): Promise<TEntity> {
    const result = await this.newQueryObject()
  ...
  async create(entity: Omit<TEntity, 'objectId'>): Promise<TEntity> {
    ...
    for (const field of Object.keys(entity)) {
      data.set(field, (entity as any)[field]);
    }
  ...
  async deleteById(objectId: string) {
    const result = await AV.Object.createWithoutData(
      this._className,
      objectId
    ).destroy();
    return result;
  ...
  async deleteByIds(objectIds: string[]) {
    const result = await AV.Object.destroyAll(
      objectIds.map((objectId) =>
        AV.Object.createWithoutData(this._className, objectId)
      )
    );
    return result;
  }
