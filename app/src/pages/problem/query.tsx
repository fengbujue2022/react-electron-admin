...
const fetchData = async (page: number, rowsPerPage: number, selectedOption?: DataModel.Problem) => {
  let totalCount = 0
  ...
  try {
    ...
    if (problemsResult.data.length > 0) {
      const reporterResult = await ReporterRepository.getManyByIds(problemsResult.data.map((x) => x.reporterId))
      data = problemsResult.data.map((problem) => {
        const reporter = reporterResult.find((r) => r.objectId === problem.reporterId)
        return {
...
export default function List() {
  ...
  const [options, selectedOption, setSelectedOption, setKeyword] = useAutoComplete(
    ProblemRepository,
    'description'
  )
        ...
        <MUIDataTable
          ...
          options={{
            ...
            onRowClick: (rowData, rowMeta) => {
              history.push(generatePath(routes.ProblemEdit, { id: tableData[rowMeta.dataIndex].objectId }))
            },
