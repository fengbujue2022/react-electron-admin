...
import { ActionData } from '@/electron/types'
import { ipcMain, WebContents } from 'electron'

function finshWithResult(sender: WebContents, event_name: string, data: [any] | [any, any]) {
  try {
...
ipcMain.on('x_action', async (e, action_data: ActionData) => {
  ...
  if (typeof fn === 'function') {
    ...
    try {
      ...
      finshWithResult(sender, callbackId, [null, v])
    } catch (e) {
      console.error(e)
      finshWithResult(sender, callbackId, [e])
    }
  } else {
